...
namespace Antmicro.Renode.Integrations
{
    ...
    public class RttReader : IExternal, IUART
    {
        ...
        {
            this.NoisyLog("Creating RttReader");
        ...
        private void SearchForControlBlock() {
            ...
            this.NoisyLog("Found control block at 0x{0:X}", controlBlockAddress);
            ...
            this.NoisyLog("RTT MaxUpChannels={0}, MaxDownChannels={1}", maxUpChannels, maxDownChannels);
            if((maxUpChannels > 255) || (maxDownChannels > 255))
            {
                throw new RecoverableException("RTT up/down channels are invalid");
            }

            // TODO - this is defmt specific
            if((maxUpChannels != 1) || (maxDownChannels != 0))
            {
                throw new RecoverableException("RTT reader only supports channel config up=1 down=0");
            }

            // Read the reset of the control block (just up channel 0)
            var chAddr = controlBlockAddress + ChannelArraysOffset;
            // Read up channel 0
            ...
            this.NoisyLog("RTT channel 0 : name=0x{0:X}, buffer=0x{1:X}, size={2}, write={3}, read={4}, flags=0x{5:X}",
